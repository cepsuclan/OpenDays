...
const scenes = {
  start: {
    text: ">> Booting system...\n>> Incoming alerts...\n\nYou're a junior cybersecurity analyst working at YuClan Computing. Multiple alerts are queued. What do you do?",
    options: [
      ...
      { text: ">> Review flagged employee activity", next: "insiderThreat", explanation: "Good instinct — insider threats are often overlooked but can cause serious damage.", isGood: true },
      { text: ">> Check the APT alert from overnight", next: "apthack1", explanation: "Correct priority — Advanced Persistent Threats require immediate attention.", isGood: true },
      { text: ">> Investigate unauthorized software alert", next: "shadowIT", explanation: "Smart — shadow IT and unauthorized software are common entry points for attackers.", isGood: true },
      { text: ">> Review the cloud storage misconfiguration alert", next: "misconfiguredCloudBucket", explanation: "Good — cloud misconfigurations can expose sensitive data to the public internet.", isGood: true },
      { text: ">> Check the credential stuffing alert on the login portal", next: "credentialStuffing", explanation: "Important — credential stuffing can compromise many customer accounts quickly.", isGood: true },
      { text: ">> Review the zero-day vulnerability report", next: "zerodayexploit", explanation: "Urgent — zero-day exploits have no patch available and need immediate containment.", isGood: true }
    ]
  },
  apthack1: {
  ...
  email: {
    ...
    options: [
      ...
      { text: ">> Warn all staff immediately about suspicious emails", next: "hover", explanation: "Good instinct — broad staff awareness can stop others falling for the same campaign while the team investigates.", isGood: true }
    ]
  },
  aptscan2: {
  ...
  networkTraffic: {
    ...
    options: [
      ...
      { text: ">> Ignore (could be false positive)", next: "fail", explanation: "Incorrect — ignoring could allow data to be exfiltrated undetected.",
        isGood: false }
    ]
  },
  traceSuccess: {
  ...
  insiderThreat: {
    ...
    options: [
      ...
      { text: ">> Report to HR and initiate investigation", next: "insiderinvestigate", explanation: "Good — HR and IT need to collaborate to protect the company from internal threats.", isGood: true }
    ]
  },
  shadowIT: {
  ...
  emailMonitoring: {
    text: ">> Email monitoring is active. Over the next 48 hours, three more phishing campaigns are caught early and blocked before reaching staff inboxes.\n\n>> Your proactive stance has significantly reduced the organization's phishing exposure.",
    options: [
      { text: ">> Schedule a post-incident review", next: "postIncidentReview", explanation: "Good — reviewing the incident cements lessons learned and improves future defences.", isGood: true },
      { text: ">> Return to normal operations — threat contained", next: "win", explanation: "Reasonable — the immediate threat is resolved and monitoring is in place.", isGood: true }
    ]
  },
  phishingCampaignInvestigation: {
    text: ">> Deeper investigation reveals this is a coordinated spear-phishing campaign targeting five departments.\n>> Attackers are impersonating the CFO to request urgent wire transfers and credential updates.",
    options: [
      { text: ">> Alert all departments and enforce multi-factor authentication immediately", next: "win", explanation: "Excellent — MFA is one of the most effective controls against credential-based attacks. Broad alerting prevents further victims.", isGood: true },
      { text: ">> Block the sender domain and notify only the affected departments", next: "emailMonitoring", explanation: "Partially correct — domain blocking helps, but the campaign may pivot to new domains. Wider awareness is also needed.", isGood: true },
      { text: ">> Forward findings to management and wait for approval before acting", next: "fail", explanation: "Incorrect — active spear-phishing campaigns require immediate action. Delay allows more staff to be compromised.", isGood: false }
    ]
  },
  stakeholderNotification: {
    text: ">> You notify senior management, the legal team, and the Data Protection Officer about the incident.\n>> Legal confirms a regulatory notification obligation under GDPR — the ICO must be informed within 72 hours if personal data was affected.",
    options: [
      { text: ">> Prepare and submit the ICO breach notification within the 72-hour window", next: "win", explanation: "Correct — GDPR Article 33 requires notification to the supervisory authority within 72 hours of becoming aware of a breach involving personal data.", isGood: true },
      { text: ">> Conduct a post-incident review before deciding whether to notify", next: "postIncidentReview", explanation: "Caution — the review is valuable, but it must not delay a mandatory regulatory notification. Both should run in parallel.", isGood: true },
      { text: ">> Decide personal data was not affected and take no further action", next: "fail", explanation: "Incorrect — failing to properly assess and notify can result in substantial regulatory fines and reputational damage.", isGood: false }
    ]
  },
  emailFiltering: {
    text: ">> Stricter email filtering rules are deployed — SPF, DKIM and DMARC records are enforced, and attachment sandboxing is enabled.\n>> Within one week, automated filtering blocks 94% of suspicious inbound emails before they reach user inboxes.",
    options: [
      { text: ">> Pair the filtering improvements with employee awareness training", next: "awarenessTraining", explanation: "Perfect combination — technical controls and human awareness together provide defence-in-depth.", isGood: true },
      { text: ">> Mark the remediation complete and return to monitoring", next: "win", explanation: "Good — the technical hardening is solid. Ongoing monitoring will catch anything that slips through.", isGood: true }
    ]
  },
  aptscan: {
    text: ">> Company-wide email scan complete. Seventeen malicious attachments identified across nine mailboxes — all quarantined before execution.\n>> Analysis reveals the attachments contained a dropper designed to establish persistent backdoor access.",
    options: [
      { text: ">> Check all nine affected machines for signs of compromise", next: "aptscan2", explanation: "Correct — even with attachments quarantined, machines may have been compromised through other vectors. Verify each one.", isGood: true },
      { text: ">> Assume quarantine was sufficient and resume normal operations", next: "fail", explanation: "Incorrect — quarantine only caught the email stage. If any attachment was opened before quarantine, the host may already be compromised.", isGood: false }
    ]
  },
  aptscan3: {
    text: ">> Lateral movement has been locked down. A network scan identifies two unauthorized devices — one appearing to be a rogue access point, one an unregistered workstation.\n>> Both are isolated. Forensic imaging begins.",
    options: [
      { text: ">> Perform full forensic analysis on both devices to determine scope", next: "win", explanation: "Excellent — forensics will reveal how long the attacker has been present, what data was accessed, and how the rogue devices were introduced.", isGood: true },
      { text: ">> Wipe and remove both devices immediately", next: "fail", explanation: "Incorrect — wiping destroys forensic evidence. You must image and preserve before any remediation.", isGood: false }
    ]
  },
  zerodayContainment: {
    text: ">> The vendor acknowledges the vulnerability and begins developing a patch — estimated 5 days away.\n>> You implement a temporary workaround: disabling the affected feature and deploying enhanced monitoring on all hosts running the vulnerable version.",
    options: [
      { text: ">> Apply the vendor patch immediately when released and verify all hosts", next: "win", explanation: "Correct — applying the patch closes the vulnerability. Verification ensures no host is missed.", isGood: true },
      { text: ">> Keep the workaround in place indefinitely — it seems to be working", next: "fail", explanation: "Incorrect — workarounds are temporary mitigations, not permanent fixes. The underlying vulnerability remains exploitable until patched.", isGood: false }
    ]
  },
  win: {
